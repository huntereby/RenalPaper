...
res_1_week <- subset(res_1_week, select=c("GeneID","padj","pvalue","stat","baseMean","Symbol","Description"))
#3 month comparison
...
res_3_week <- subset(res_3_week, select=c("GeneID","padj","pvalue","stat","baseMean","Symbol","Description"))
#6 month comparison
...
res_6_week <- subset(res_6_week, select=c("GeneID","padj","pvalue","stat","baseMean","Symbol","Description"))
...
plotDispEsts(ds, main="GSE86884 Dispersion Estimates")

# create histogram plot of p-values
hist(r$padj, breaks=seq(0, 1, length = 21), col = "grey", border = "white", 
     xlab = "", ylab = "", main = "GSE86884 Frequencies of padj-values")

# Wald test to obtain contrast-specific results
ds <- DESeq(ds, test="Wald", sfType="poscount")
r <- results (ds, contrast=c("Group", groups[1], groups[4]), alpha=0.05, pAdjustMethod = "fdr")

# volcano plot
old.pal <- palette(c("#00BFFF", "#FF3030")) # low-hi colors
par(mar=c(4,4,2,1), cex.main=1.5)
plot(r$log2FoldChange, -log10(r$padj), main=paste(groups[1], "vs", groups[4]),
...
par(mar=c(7,4,2,1))
boxplot(dat[,ord], boxwex=0.6, notch=T, main="GSE86884", ylab="lg(norm.counts)", outline=F, las=2, col=gs[ord])
